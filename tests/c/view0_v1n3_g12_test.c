...
int main(void)
{
    static const char html[] = "<!doctype html><html><head><title>x</title><script type=module nomodule></script></head><body></body></html>";
    ...
    for (uint64_t i = 0u; i < result.diagnostic_count; ++i)
        if (diagnostics[i].rule_id == ARBOR_VIEW0_NATIVE_RULE_ID_V1N3_G12_R2) found = true;
    if (!found) return 2;
    puts("VIEW0_V1N3_G12_FUNCTIONAL=PASS");
    puts("VIEW0_V1N3_G12_RULES=8_OF_8_BOUND");
    return 0;
}
