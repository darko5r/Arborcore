...
int main(void)
{
    static const char html[] = "<!doctype html><html><head><title>x</title></head><body><button onclick='if ('>x</button></body></html>";
    ...
    for (uint64_t i = 0u; i < result.diagnostic_count; ++i)
        if (diagnostics[i].rule_id == ARBOR_VIEW0_NATIVE_RULE_ID_V1N3_G16_R1) found = true;
    if (!found) return 2;
    puts("VIEW0_V1N3_G16_FUNCTIONAL=PASS");
    puts("VIEW0_V1N3_G16_RULES=2_OF_2_BOUND");
    return 0;
}
