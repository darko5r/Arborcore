...
while IFS= read -r path; do
  [[ -n "$path" ]] || continue
  printf '%s\n' "$actual" | grep -Fxq "$path" || { echo "FAIL: required VIEW0 C4 path missing from cumulative candidate: $path" >&2; exit 1; }
done <<< "$required"
printf 'VIEW0_C4_REQUIRED_PATH_COUNT=34\nVIEW0_C4_CUMULATIVE_PATH_COUNT=%s\n' "$(printf '%s\n' "$actual" | sed '/^$/d' | wc -l)"
echo 'VIEW0_C4_STAGED_CHANGES=NO'
echo 'PASS: VIEW0 C4 required construction paths remain present under cumulative VIEW0 extension'
