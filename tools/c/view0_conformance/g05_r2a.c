...
typedef struct g05_r2a_context {
...
_Static_assert(sizeof("ARBOR_VIEW_V1_G05_ELEMENT_ATTRIBUTE_APPLICABILITY") <=
                   ARBOR_VIEW0_NATIVE_RULE_SYMBOL_CAP,
               "G05 R2 symbolic name exceeds diagnostic capacity");
_Static_assert(sizeof("Attribute name is not admitted for this element by the frozen element-specific applicability catalog") <=
                   ARBOR_VIEW0_NATIVE_MESSAGE_CAP,
               "G05 R2 message exceeds diagnostic capacity");
    ...
    const arbor_view0_native_source_attribute_observation *observation)
{
    ...
    g05_r2a_context *context = (g05_r2a_context *)opaque;
    ...
    if (arbor_view0_native_g05_c0_global_attribute_classify(observation->local_name, &kind)) {
        if (context->global_attribute_handoff_count == UINT64_MAX) return err_status(EOVERFLOW);
        context->global_attribute_handoff_count += 1u;
        return ok_status();
    }

    /* R4 owns the frozen body Window-event placement rule, including rejection off body. */
    if (arbor_view0_native_g05_c0_body_window_event_listed(observation->local_name)) {
        if (context->r4_body_event_handoff_count == UINT64_MAX) return err_status(EOVERFLOW);
        context->r4_body_event_handoff_count += 1u;
        return ok_status();
    }

    /* Static list membership is R2. Conditional state/prerequisite predicates remain R3-owned. */
    if (arbor_view0_native_g05_c0_element_attribute_listed(
            observation->owner_standard_element_id, observation->local_name)) {
        if (context->admitted_element_attribute_count == UINT64_MAX) return err_status(EOVERFLOW);
        context->admitted_element_attribute_count += 1u;
        return ok_status();
    }

    if (!arbor_view0_native_g05_c0_known_non_global_attribute_name(observation->local_name)) {
        /* Unknown names are already owned by R1; do not duplicate its diagnostic. */
        if (context->r1_owned_unknown_count == UINT64_MAX) return err_status(EOVERFLOW);
        context->r1_owned_unknown_count += 1u;
        return ok_status();
    }
    ...
    arbor_view0_native_g05_r2a_evaluation *evaluation_out)
{
    ...
    *evaluation_out = (arbor_view0_native_g05_r2a_evaluation){
        .diagnostic_count = context.diagnostic_count,
        .admitted_element_attribute_count = context.admitted_element_attribute_count,
        .global_attribute_handoff_count = context.global_attribute_handoff_count,
        .r1_owned_unknown_count = context.r1_owned_unknown_count,
        .r4_body_event_handoff_count = context.r4_body_event_handoff_count,
...
{
    static const char symbolic[] = "ARBOR_VIEW_V1_G05_ELEMENT_ATTRIBUTE_APPLICABILITY";
    static const char message[] =
        "Attribute name is not admitted for this element by the frozen element-specific applicability catalog";
    if (anchor == NULL || diagnostic == NULL) return;
    (void)memset(diagnostic, 0, sizeof(*diagnostic));
    diagnostic->rule_id = ARBOR_VIEW_V1_G05_ELEMENT_ATTRIBUTE_APPLICABILITY;
