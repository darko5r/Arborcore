...
while IFS= read -r path; do
  [[ -n "$path" ]] || continue
  printf '%s\n' "$actual" | grep -Fxq "$path" || { echo "FAIL: required VIEW0 C1 path missing from cumulative candidate: $path" >&2; exit 1; }
done <<< "$required"
echo 'VIEW0_C1_REQUIRED_PATH_COUNT=12'
echo 'VIEW0_C1_EXTENSION_AWARE_SCOPE=YES'
echo 'VIEW0_C1_STAGED_CHANGES=NO'
echo 'PASS: VIEW0 C1 required construction paths remain present in cumulative VIEW0 candidate'
