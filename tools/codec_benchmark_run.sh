...
RUNS="${ARBORCORE_CODEC_RUNS:-31}"
WARMUPS="${ARBORCORE_CODEC_WARMUPS:-3}"
CPU="$(arborcore_bench_cpu)"
PROFILE="$(arborcore_perf_profile)"
OUT_DIR="${ARBORCORE_CODEC_OUT_DIR:-$ROOT/build/codec-benchmark-$PROFILE}"

if ! [[ "$RUNS" =~ ^[1-9][0-9]*$ ]] || ! [[ "$WARMUPS" =~ ^[0-9]+$ ]]; then
    echo "Invalid codec benchmark run/warmup count" >&2
    exit 2
fi
...
if [[ "$production_tree_state" == dirty \
      && "${ARBORCORE_CODEC_ALLOW_DIRTY_PRODUCTION:-0}" != 1 ]]
then
    echo "FAIL: production src/asm has local changes." >&2
    echo "Reference/committed codec benchmarking requires a clean production tree." >&2
    echo "Use verify-codec-performance-candidate for deliberate pre-commit candidates." >&2
    exit 2
fi

if [[ "$production_tree_state" == dirty ]]; then
    echo "NOTICE: codec candidate mode allows dirty production; exact source identity will be recorded."
fi

BIN="$ROOT/build/bench-codec"
[[ -x "$BIN" ]] || { echo "Missing benchmark executable: build/bench-codec" >&2; exit 2; }
...
percent_text="$(size "$ROOT/build/percent_codec.o" | awk 'NR==2{print $1}')"

{
    ...
    echo "percent_codec_text_bytes=$percent_text"
...
for ((run=1; run<=RUNS; run++)); do
    ...
    while IFS=$'\t' read -r metric iterations total_ns extra; do
        ...
        if [[ -n "${extra:-}" ]] || ! [[ "$iterations" =~ ^[0-9]+$ ]] || ! [[ "$total_ns" =~ ^[0-9]+$ ]]; then
            echo "Malformed codec benchmark result: $metric $iterations $total_ns ${extra:-}" >&2
            exit 3
        fi
...
median_of_sorted_file() {
    awk '{ a[NR]=$1 } END { n=NR; if(n==0) exit 1; if(n%2) printf "%.6f",a[(n+1)/2]; else printf "%.6f",(a[n/2]+a[n/2+1])/2 }' "$1"
}
rank_value() {
    local file="$1" pct="$2"
    awk -v p="$pct" '{a[NR]=$1} END{n=NR;if(n==0)exit 1;idx=int(p*n);if(idx<p*n)idx++;if(idx<1)idx=1;if(idx>n)idx=n;printf "%.6f",a[idx]}' "$file"
}
...
while IFS= read -r metric; do
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$metric" "$samples" "$median" "$mad" "$mad_pct" "$p90" "$p95" "$p99" "$min" "$max" "$ops" "$p95_slow" >> "$SUMMARY"
    rm -f "$values" "$devs"
