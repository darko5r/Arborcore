...
echo '### G04 R1A — RETAINED G02/G03 THROUGH R7A'
bash tools/view0_v1n1_g03_r7a_native_verify.sh

echo '### G04 R1A — FUNCTIONAL / ADVERSARIAL / GLOBAL ATOMICITY'
make -s view0-v1n1-g04-r1a-test view0-v1n1-g04-r1a-adversarial-test \
  view0-v1n1-g04-r1a-global-failure-atomicity-test
[[ -x build/view0-v1/native/g04-r1a-transparent-parent-model-test ]] || fail 'G04 R1A functional binary missing'
[[ -x build/view0-v1/native/g04-r1a-transparent-parent-model-adversarial-test ]] || fail 'G04 R1A adversarial binary missing'
[[ -x build/view0-v1/native/g04-r1a-global-failure-atomicity-test ]] || fail 'G04 R1A atomicity binary missing'

tmp=$(mktemp -d /tmp/arborcore-g04-r1a-native.XXXXXX); trap 'rm -rf "$tmp"' EXIT

echo '### G04 R1A — GCC ANALYZER'
...
echo 'VIEW0_V1N1_G04_R1A_GCC_FANALYZER=PASS'

echo '### G04 R1A — COMPILED STACK / EXISTING 900000-BYTE PHASED ADMISSION'
...
bound=$(sed -n 's/^VIEW0_V1N1_G04_R1A_PHASED_COMPILED_STACK_BOUND_BYTES=//p' view/arborcore-view-core-1.contract | head -1)
...
# Conservative deepest nested prior-owner phase:
# native -> G04 public/evaluate -> prior-owner collection -> G03 R5 -> its prior R1 -> Lexbor.
...
(( prior_phase <= bound )) || fail "G04 R1A prior-owner phased stack exceeded observed=$prior_phase bound=$bound"
# Transparent-resolution phase occurs after prior-owner collection returns.
transparent_phase=$((native_check + g04_public + g04_eval + g04_with_prior + lex_observe + lex_process + 80))
(( transparent_phase <= bound )) || fail "G04 R1A transparent phase stack exceeded observed=$transparent_phase bound=$bound"
...
echo "VIEW0_V1N1_G04_R1A_EVALUATE_WITH_PRIOR_COMPILED_STACK_BYTES=$g04_with_prior"
echo "VIEW0_V1N1_G04_R1A_PRIOR_OWNER_PHASED_COMPILED_STACK_OBSERVED_BYTES=$prior_phase"
echo "VIEW0_V1N1_G04_R1A_TRANSPARENT_PHASED_COMPILED_STACK_OBSERVED_BYTES=$transparent_phase"
echo "VIEW0_V1N1_G04_R1A_PHASED_COMPILED_STACK_BOUND_BYTES=$bound"
echo 'VIEW0_V1N1_G04_R1A_STACK_THRESHOLD_WIDENING=NO'

echo '### G04 R1A — CLI SOURCE-REPAIR / DEFERRAL CONTROLS'
...
grep -Fq $'error\t0x0000000030040001\tARBOR_VIEW_V1_G04_TRANSPARENT_PARENT_MODEL\tarborcore-authoring' "$tmp/matrix_negative.tsv" || fail 'frozen negative G04 diagnostic missing'
...
grep -Fq $'g04_r1=partial' "$tmp/matrix_negative.tsv" || fail 'G04 partial CLI marker missing'
...
if [[ "$current_version" == '0.1-VIEW0-V1N1-G04-R1C' || "$current_version" == '0.1-VIEW0-V1N1-G04-R2' ]]; then
  grep -Fq $'g04_r1_noscript_scripting_deferred=no' "$tmp/noscript.tsv" || fail 'R1C noscript deferral was not retired'
else
  grep -Fq $'g04_r1_noscript_scripting_deferred=yes' "$tmp/noscript.tsv" || fail 'noscript deferral marker missing'
fi
...
if [[ "$current_version" == '0.1-VIEW0-V1N1-G04-R1A' ]]; then
  grep -Fq $'g04_r1_option_branch_deferred=yes' "$tmp/option.tsv" || fail 'R1A option branch deferral marker missing'
else
  grep -Fq $'g04_r1_option_branch_deferred=no' "$tmp/option.tsv" || fail 'R1B option branch deferral was not retired'
fi
run_cli custom '<!doctype html><title>x</title><body><x-r1><div>x</div></x-r1></body>' 0
grep -Fq $'g04_r1_g13_custom_deferred=yes' "$tmp/custom.tsv" || fail 'G13 custom deferral marker missing'
echo 'VIEW0_V1N1_G04_R1A_CLI_FROZEN_MATRIX=2_OF_2'
echo 'VIEW0_V1N1_G04_R1A_CLI_NEGATIVE_ANCHOR=PASS_OFFSET_53_LENGTH_3_LINE_1_COLUMN_54'
echo "VIEW0_V1N1_G04_R1A_CLI_EXTENSION_STATE=$current_version"
echo 'PASS: G04 R1A native core retained under current admitted extension'
