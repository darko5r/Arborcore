...
while IFS= read -r path; do
  [[ -n "$path" ]] || continue
  grep -Fxq "$path" <<< "$actual" || { echo "FAIL: missing required VIEW0 M1 path under extension: $path" >&2; exit 1; }
done <<< "$required"
printf 'VIEW0_M1_REQUIRED_PATH_COUNT=%s\n' "$(printf '%s\n' "$required" | sed '/^$/d' | wc -l)"
printf 'VIEW0_M1_CUMULATIVE_PATH_COUNT=%s\n' "$(printf '%s\n' "$actual" | sed '/^$/d' | wc -l)"
echo 'VIEW0_M1_EXTENSION_AWARE_SCOPE=YES'
echo 'VIEW0_M1_STAGED_CHANGES=NO'
echo 'PASS: VIEW0 M1 required construction paths remain present under cumulative VIEW0 extension'
