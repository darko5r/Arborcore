...
while IFS= read -r path; do
  [[ -n "$path" ]] || continue
  grep -Fxq "$path" <<< "$actual" || { echo "FAIL: missing required VIEW0 T1 path under extension: $path" >&2; exit 1; }
done <<< "$expected"
printf 'VIEW0_T1_REQUIRED_PATH_COUNT=41\n'
printf 'VIEW0_T1_CUMULATIVE_PATH_COUNT=%s\n' "$(printf '%s\n' "$actual" | sed '/^$/d' | wc -l)"
echo 'VIEW0_T1_STAGED_CHANGES=NO'
echo 'PASS: VIEW0 T1 required construction paths remain present under cumulative VIEW0 extension'
