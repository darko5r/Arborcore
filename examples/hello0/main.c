...
#define HELLO0_CONFIG_FILE_CAPACITY 8192u
#define HELLO0_CONFIG_CLI_CAPACITY 6u
#define HELLO0_CONFIG_PERSISTENT_CAPACITY 4110u
#define HELLO0_CONFIG_ENV_ENTRY_CAPACITY 8256u
#define HELLO0_TEMPLATE_PATH_CAPACITY 4096u
#define HELLO0_BIND_IPV4_CAPACITY 16u
...
static const arbor_config_descriptor hello0_config_descriptors[] = {
    {
...
static void hello0_config_error(const arbor_config_diagnostic *diagnostic)
{
    fprintf(
        stderr,
        "HELLO0_CONFIG_ERROR code=%u source=%u record=%" PRIu64
...
static int hello0_add_environment(
    const char *name,
    char *storage,
    size_t capacity,
...
static int hello0_acquire_configuration(
    int argc,
    char **argv,
    arbor_config_sources *sources_out,
    uint8_t file_storage[HELLO0_CONFIG_FILE_CAPACITY],
    arbor_span environment_entries[6],
    char environment_storage[6][HELLO0_CONFIG_ENV_ENTRY_CAPACITY],
    arbor_span command_line_entries[HELLO0_CONFIG_CLI_CAPACITY],
    char legacy_template[HELLO0_CONFIG_ENV_ENTRY_CAPACITY],
    char legacy_port[96])
{
    ...
    if (legacy) {
        ...
        if (template_length >
                HELLO0_CONFIG_ENV_ENTRY_CAPACITY - sizeof(template_prefix) ||
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (strncmp(argv[index], prefix, sizeof(prefix) - 1u) == 0) {
            ...
            if (strncmp(argv[index], "--", 2u) != 0 ||
                command_line_count >= HELLO0_CONFIG_CLI_CAPACITY) {
                return 1;
            }
    ...
    if (config_path != NULL &&
        hello0_read_file(
            config_path, file_storage, HELLO0_CONFIG_FILE_CAPACITY, false,
            &file_document) != 0) {
        fputs("HELLO0_CONFIG_FILE_ERROR\n", stderr);
        return 1;
    }
    ...
    for (uint64_t index = 0u; index < 6u; ++index) {
        bool present = false;
        arbor_span entry;
        if (hello0_add_environment(
                environment_names[index], environment_storage[index],
                HELLO0_CONFIG_ENV_ENTRY_CAPACITY, &entry, &present) != 0) {
            fputs("HELLO0_CONFIG_ENVIRONMENT_ERROR\n", stderr);
            return 1;
        }
        if (present) {
...
{
    static uint8_t config_file[HELLO0_CONFIG_FILE_CAPACITY];
    static arbor_span environment_entries[6];
    static char environment_storage[6][HELLO0_CONFIG_ENV_ENTRY_CAPACITY];
    static arbor_span command_line_entries[HELLO0_CONFIG_CLI_CAPACITY];
    static char legacy_template[HELLO0_CONFIG_ENV_ENTRY_CAPACITY];
    static char legacy_port[96];
    arbor_config_sources config_sources;
    if (hello0_acquire_configuration(
            argc, argv, &config_sources, config_file, environment_entries,
            environment_storage, command_line_entries, legacy_template,
            legacy_port) != 0) {
        fputs("HELLO0_CONFIG_ARGUMENT_ERROR\n", stderr);
        return 2;
    }
    ...
    arbor_status status = arbor_config_measure(
        &hello0_config_schema, &config_sources, &requirements,
        &config_diagnostic);
    if (status.native != 0) {
        hello0_config_error(&config_diagnostic);
        return 2;
    }
    ...
    static uint8_t config_persistent[HELLO0_CONFIG_PERSISTENT_CAPACITY];
    ...
    status = arbor_config_prepare(
        &hello0_config_schema, &config_sources, &config_storage,
        &config_result, &config_diagnostic);
    if (status.native != 0 ||
        arbor_config_validate(&hello0_config_schema, &config_result).native !=
            0) {
        hello0_config_error(&config_diagnostic);
        return 2;
    }
    if (requirements.persistent_bytes > sizeof(config_persistent)) {
        fputs("HELLO0_CONFIG_CAPACITY_ERROR\n", stderr);
        return 2;
    }

    char template_path[HELLO0_TEMPLATE_PATH_CAPACITY];
    char bind_ipv4[HELLO0_BIND_IPV4_CAPACITY];
    if (hello0_copy_text(
            config_values[0].utf8_value, template_path,
            sizeof(template_path)) != 0 ||
        hello0_copy_text(
            config_values[1].utf8_value, bind_ipv4, sizeof(bind_ipv4)) != 0) {
        fputs("HELLO0_CONFIG_MAPPING_ERROR\n", stderr);
        return 2;
    }
    ...
    if (inet_pton(AF_INET, bind_ipv4, &address.sin_addr) != 1) {
        fputs("HELLO0_CONFIG_IPV4_ERROR\n", stderr);
        return 2;
    }
    ...
    if (hello0_read_file(
            ...
            &source) != 0) {
        fputs("HELLO0_TEMPLATE_READ_ERROR\n", stderr);
        return 1;
    }
    static hello0_web_application application;
    status = hello0_web_application_prepare(
        source, HELLO0_RESPONSE_FIELD_CAPACITY, &application);
    if (status.native != 0) {
    ...
    static arbor_linux_http_mvc_host_slot slots[HELLO0_CONNECTION_SLOT_COUNT];
    ...
    static uint8_t slot_arenas
        [HELLO0_CONNECTION_SLOT_COUNT][HELLO0_CONNECTION_BUFFER_CAPACITY];
    for (size_t index = 0u; index < HELLO0_CONNECTION_SLOT_COUNT; ++index) {
    ...
    status = arbor_linux_http_mvc_host_options_make(
        ...
        hello0_host_diagnostic, NULL);
    if (status.native != 0) {
        fprintf(stderr, "HELLO0_HOST_OPTIONS_ERROR=%" PRId64 "\n", status.native);
        return 1;
    }
    arbor_linux_http_mvc_host host = {0};
    status = arbor_linux_http_mvc_host_prepare(
        &host, &application.http_application, slots,
        HELLO0_CONNECTION_SLOT_COUNT, events, HELLO0_EVENT_CAPACITY,
        &host_options);
    if (status.native != 0) {
    ...
    printf("HELLO0_READY=http://%s:%" PRIu16 "/hello\n", bind_ipv4, port);
    (void)fflush(stdout);
    arbor_status run_status = arbor_linux_http_mvc_host_run(
        &host, hello0_should_stop, NULL);
