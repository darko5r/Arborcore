...
#define ECHO0_BENCH_MAX_ROUTES 256u
#define ECHO0_BENCH_PATTERN_CAPACITY 8u
#define ECHO0_BENCH_WORKSPACE_CAPACITY 8u
#define ECHO0_BENCH_BUFFER_CAPACITY 16384u
#define ECHO0_BENCH_REQUESTS_PER_ROUND 64u
#define ECHO0_BENCH_QUIESCENT_STEPS 1000u
#define ECHO0_BENCH_ROUNDS 9u

typedef struct echo0_bench_application {
    uint8_t patterns[ECHO0_BENCH_MAX_ROUTES][ECHO0_BENCH_PATTERN_CAPACITY];
    arbor_mvc_route routes[ECHO0_BENCH_MAX_ROUTES];
    arbor_mvc_catalog catalog;
    arbor_mvc_application mvc;
    arbor_http_mvc_application http;
} echo0_bench_application;

static int64_t echo0_bench_controller(
...
static int64_t echo0_bench_presenter(
    const arbor_mvc_request *request,
    void *context,
...
static int echo0_bench_prepare(
    echo0_bench_application *application,
    uint64_t route_count)
{
    static const uint8_t method[] = "GET";
    if (application == NULL || route_count == 0u ||
        route_count > ECHO0_BENCH_MAX_ROUTES) {
        return 1;
    }
    ...
    for (uint64_t i = 0u; i < route_count; ++i) {
        ...
        application->routes[i] = (arbor_mvc_route){
            ...
            (uint64_t)length,
            echo0_bench_controller,
            NULL,
            echo0_bench_presenter,
            NULL,
            NULL,
            0u
        };
    ...
    arbor_route_param params[ECHO0_BENCH_WORKSPACE_CAPACITY] = {0};
    arbor_mvc_prepare_workspace workspace = {
        params,
        ECHO0_BENCH_WORKSPACE_CAPACITY
    };
...
static int echo0_bench_now(uint64_t *nanoseconds_out)
{
...
static int echo0_bench_write_all(int fd, const uint8_t *data, size_t length)
{
    size_t offset = 0u;
    while (offset < length) {
...
static int echo0_bench_make_pipeline(
    const char *path,
    uint8_t *buffer,
    size_t capacity,
    size_t *length_out)
{
    ...
    size_t used = 0u;
    for (uint64_t i = 0u; i < ECHO0_BENCH_REQUESTS_PER_ROUND; ++i) {
        const char *connection =
            i + 1u == ECHO0_BENCH_REQUESTS_PER_ROUND ?
                "Connection: close\r\n" : "";
        int length = snprintf(
...
static int echo0_bench_pipeline_round(
    const echo0_bench_application *application,
    const char *path,
    uint64_t *nanoseconds_per_request_out)
{
    uint8_t requests[ECHO0_BENCH_BUFFER_CAPACITY] = {0};
    size_t request_length = 0u;
    if (echo0_bench_make_pipeline(
            path,
            requests,
            sizeof(requests),
            &request_length) != 0) {
    ...
    uint8_t input[ECHO0_BENCH_BUFFER_CAPACITY] = {0};
    uint8_t output[ECHO0_BENCH_BUFFER_CAPACITY] = {0};
    uint8_t arena[ECHO0_BENCH_BUFFER_CAPACITY] = {0};
    ...
    if (arbor_runtime_storage_prepare(
        ...
        echo0_bench_write_all(sockets[0], requests, request_length) != 0) {
    ...
    if (echo0_bench_now(&finished) != 0 || finished < started ||
        completed != ECHO0_BENCH_REQUESTS_PER_ROUND) {
        (void)close(sockets[0]);
        return 1;
    }
    (void)close(sockets[0]);
    *nanoseconds_per_request_out =
        (finished - started) / ECHO0_BENCH_REQUESTS_PER_ROUND;
    return 0;
}

static int echo0_bench_quiescent_round(
    const echo0_bench_application *application,
    uint64_t *nanoseconds_per_step_out)
{
    ...
    for (uint64_t i = 0u; i < ECHO0_BENCH_QUIESCENT_STEPS; ++i) {
        arbor_status status = arbor_http_mvc_server_step(
            &storage,
            &application->http,
            -1,
    ...
    *nanoseconds_per_step_out =
        (finished - started) / ECHO0_BENCH_QUIESCENT_STEPS;
    return 0;
}

static uint64_t echo0_bench_median(uint64_t values[ECHO0_BENCH_ROUNDS])
{
    for (size_t i = 1u; i < ECHO0_BENCH_ROUNDS; ++i) {
    ...
    return values[ECHO0_BENCH_ROUNDS / 2u];
}

static int echo0_bench_report_pipeline(
    const echo0_bench_application *application,
    uint64_t route_count,
    const char *case_name,
    const char *path)
{
    uint64_t samples[ECHO0_BENCH_ROUNDS] = {0};
    for (size_t round = 0u; round < ECHO0_BENCH_ROUNDS; ++round) {
        if (echo0_bench_pipeline_round(
                application,
                path,
                &samples[round]) != 0) {
            return 1;
    ...
    printf(
        ...
        route_count,
        case_name,
        echo0_bench_median(samples));
    return 0;
}

static int echo0_bench_report_quiescent(
    const echo0_bench_application *application,
    uint64_t route_count)
{
    uint64_t samples[ECHO0_BENCH_ROUNDS] = {0};
    for (size_t round = 0u; round < ECHO0_BENCH_ROUNDS; ++round) {
        if (echo0_bench_quiescent_round(application, &samples[round]) != 0) {
            return 1;
        }
    }
    printf(
...
int main(void)
{
    ...
    puts("ECHO0_ROUTE_SCALE_DIAGNOSTIC");
    ...
    for (size_t i = 0u; i < sizeof(route_counts) / sizeof(route_counts[0]); ++i) {
        ...
        echo0_bench_application application = {0};
        if (echo0_bench_prepare(&application, route_count) != 0) {
            fputs("FAIL: prepare route-scale application\n", stderr);
            return 1;
        }
        char last_path[ECHO0_BENCH_PATTERN_CAPACITY] = {0};
        int length = snprintf(
            last_path,
            sizeof(last_path),
            "/r%03" PRIu64,
        ...
        if (length != 5 ||
            ...
            echo0_bench_report_pipeline(
                ...
                "miss",
                "/miss") != 0 ||
            echo0_bench_report_quiescent(&application, route_count) != 0) {
            fputs("FAIL: route-scale measurement\n", stderr);
            return 1;
        }
    }
    puts("THRESHOLD_GATE=NONE_DIAGNOSTIC_BASELINE_ONLY");
    puts("PASS: ECHO0 route-scale server-step diagnostic completed");
    return 0;
}
